...
type
  ...
  TLexeme = (lxUndef { # } , lxName { a } , lxNumber { 10 } , lxPlus { + } , lxMinus { - } , lxAsterisk { * } , lxSlash
    ...
    { and } , lxOr { or } , lxXor { xor } , lxArr { arr } , lxLess { < } , lxLessOrEqual { <= } , lxGreat
    { > } , lxGreatOrEqual { >= } , lxEqual { = } , lxNotEqual { <> } , lxSemicolon { ; } , lxComma
    { , } , lxAssignment { := } , lxString { 'qwe' } , lxLSBracket { [ } , lxRSBracket { ] } , lxLBrace
    (* { *) , lxRBrace (* } *) , lxQuestion { ? } , lxAmpersand { & } );
...
const
  ...
  begin
    if LexCount = LexMax then
      raise ECheckError.Create('Слишком длинноe выражение (больше чем ' + IntToStr(LexMax) + ' элементов)');
    Inc(LexCount);
    Lexemes^[LexCount]._Type := Lex;
    Lexemes^[LexCount].Value := Val;
    if LexCount <> LexMax then
      Lexemes^[LexCount + 1]._Type := lxUndef;
  end;
...
begin
  ...
  while System.Pos(#10, tmp) > 0 do
    tmp[System.Pos(#10, tmp)] := ' '; // Delete(tmp, System.Pos(#10, tmp), 1);
  while System.Pos(#13, tmp) > 0 do
    tmp[System.Pos(#13, tmp)] := ' '; // Delete(tmp, System.Pos(#13, tmp), 1);
  ...
  repeat
    ...
    begin
      Word := '';
      while CharInSet(c, Alpha + Num) do
      begin
        Word := Word + c;
        Read;
        if c = EOFChar then
      ...
      while (Lex2Str[Lex] <> UpperCase(Word)) and (Lex <= lxArr) // проверяем на совпадение с ключевыми словами (Div..Arr)
        do
        Lex := Succ(Lex);
      if Lex2Str[Lex] = UpperCase(Word) then
        Add(Lex)
      else
        Add(lxName, Word);
    end;
    ...
    if (not CharInSet(c, Any)) or (c = ' ') then
      Read;
  until EOF;
end;
...
begin
  if not AlreadyError then
    if ExpectError then
      raise ECheckError.Create('Ожидалось ' + Expect + ', но найдено ' + Found)
    else
      raise ECheckError.Create(Expect);
  // Inc(Pos);
  AlreadyError := true;
end;
...
begin
  if not AlreadyError then
    raise ERunTimeError.Create('Ошибка времени выполнения: '#10#13'  ' + Error);
  // Inc(Pos);
  AlreadyError := true;
end;
...
begin
  ...
  begin
    ...
    if (Num > Length(_Var.Value.Str)) or (Num < 1) then
      RunTimeError('Выход за пределы строки');
    Result._Type := tyStr;
  ...
  if ArrIndex.Count = 0 then
    Result := _Var.Value
  else
  begin
    if ArrIndex.Count <> integer(_Var.Sizes.Count) then
    begin
...
begin
  ...
  if (myVar.Value._Type = tyStr) and (myVar.Sizes.Count = 0) and (ArrIndex.Count = 1) then
  begin
    if Value._Type <> tyStr then
      RunTimeError('Нельзя присвоить символам число');
    Num := integer(ArrIndex[0]^);
    if (Num + Length(Value.Str) > Length(myVar.Value.Str) + 1) or (Num < 1) then
      RunTimeError('Выход за пределы строки');
    for i := 1 to Length(Value.Str) do
      myVar.Value.Str[Num + i] := Value.Str[i];
    Exit;
  end;
  ...
  begin
    if ArrIndex.Count <> integer(myVar.Sizes.Count) then
    begin
      RunTimeError('Несоответствие количества индексов массива');
      Exit;
    end;
    ...
    PlainNo := 0;
    q := 1;
    for i := 0 to myVar.Sizes.Count - 1 do
    begin
      Inc(PlainNo, q * integer(ArrIndex[i]^));
...
procedure Expr; forward;
  ...
  begin
    ...
    begin
      ...
      begin
        Inc(Pos);
        if Lexemes^[Pos]._Type <> lxRBracket then
        begin
          Expr;
          while Lexemes^[Pos]._Type <> lxRBracket do
          begin
            if Lexemes^[Pos]._Type <> lxComma then
              CheckError(true, 'запятая или скобка', Lex2Str[Lexemes^[Pos]._Type]);
            Inc(Pos);
            Expr;
          end;
        end;
      ...
      begin
        ...
        begin
          Inc(Pos);
          Expr;
          while Lexemes^[Pos]._Type <> lxRSBracket do
          begin
            if Lexemes^[Pos]._Type <> lxComma then
              CheckError(true, 'запятая или квадратная скобка', Lex2Str[Lexemes^[Pos]._Type]);
            Inc(Pos);
            Expr;
          end;
          Inc(Pos);
    ...
    else if Lexemes^[Pos]._Type = lxNumber then
      Inc(Pos)
    else if Lexemes^[Pos]._Type in [lxPlus, lxMinus] then
    begin
      Inc(Pos);
      Element;
    end
    else if Lexemes^[Pos]._Type = lxString then
    begin
      Inc(Pos);
    end
    else if Lexemes^[Pos]._Type = lxLBracket then
    begin
      Inc(Pos);
      Expr;
      if Lexemes^[Pos]._Type = lxRBracket then
        ...
        CheckError(true, ')', Lex2Str[Lexemes^[Pos]._Type]);
    end
    else if Lexemes^[Pos]._Type = lxAmpersand then
    begin
      Inc(Pos);
      if Lexemes^[Pos]._Type = lxName then
        Inc(Pos)
  ...
  begin
    Element;
    while Lexemes^[Pos]._Type = lxPower do
    begin
      Inc(Pos);
      Element;
    end;
  ...
  begin
    Power;
    while Lexemes^[Pos]._Type in [lxAsterisk, lxSlash, lxDiv, lxMod] do
    begin
      Inc(Pos);
      Power;
    end;
  ...
  begin
    Term;
    while Lexemes^[Pos]._Type in [lxPlus, lxMinus] do
    begin
      Inc(Pos);
      Term;
    end;
  ...
  begin
    SimpleExpr;
    if Lexemes^[Pos]._Type in [lxLess, lxLessOrEqual, lxGreat, lxGreatOrEqual, lxEqual, lxNotEqual] then
    begin
      Inc(Pos);
      SimpleExpr;
    end;
  ...
  begin
    if Lexemes^[Pos]._Type = lxNot then
      Inc(Pos);
    IfExpr;
  end;
  ...
  begin
    notExpr;
    while Lexemes^[Pos]._Type = lxAnd do
    begin
      Inc(Pos);
      notExpr;
    end;
  ...
  begin
    andExpr;
    while Lexemes^[Pos]._Type in [lxOr, lxXor] do
    begin
      Inc(Pos);
      andExpr;
    end;
...
function Expr: TValue; forward;
      ...
      if Lexemes^[Pos]._Type = lxLBracket then
        ...
        begin
          ...
          begin
            ...
            New(pInt);
            tmp := Expr;
            if (tmp._Type <> tyReal) or (tmp.Real < 0) then
              RunTimeError('Индекс элемента или символа должен быть целым неотрицательным числом');
            pInt^ := Trunc(tmp.Real);
            ArrIndex.Add(pInt);
            while Lexemes^[Pos]._Type <> lxRSBracket do
            begin
              ...
              tmp := Expr;
              if (tmp._Type <> tyReal) or (tmp.Real < 0) then
                RunTimeError('Индекс элемента должен быть целым неотрицательным числом');
              pInt^ := Trunc(tmp.Real);
              ArrIndex.Add(pInt);
            end;
            Inc(Pos);
  ...
  begin
    ...
    begin
      Inc(Pos);
      tmp := Element;
      if (Result._Type <> tyReal) or (tmp._Type <> tyReal) then
        RunTimeError('Операция возведения в степень над строками недопустима');
  ...
  begin
    ...
    begin
      ...
      tmp := Power;
      if (Result._Type <> tyReal) or (tmp._Type <> tyReal) then
        RunTimeError('Недопустимая операция: Умножение/Деление строк');
      case Op of
        lxAsterisk:
        ...
        lxSlash:
          Result.Real := Result.Real / tmp.Real;
        lxDiv:
          Result.Real := Trunc(Result.Real) div Trunc(tmp.Real);
        lxMod:
          Result.Real := Trunc(Result.Real) mod Trunc(tmp.Real);
      end;
      Result._Type := tyReal;
    end;
  end;
    ...
    begin
      ...
      case Lexemes^[Pos - 1]._Type of
        lxPlus:
          begin
            tmp := Term;
            if tmp._Type <> Result._Type then
            begin
        ...
        lxMinus:
          begin
            tmp := Term;
            if (tmp._Type <> tyReal) or (Result._Type <> tyReal) then
            begin
  ...
  begin
    ...
    begin
      ...
      if Result._Type <> tmp._Type then
      begin
        RunTimeError('Недопустимая операция: Сравнение разных типов');
        Inc(Pos);
      end;

      if Result._Type = tyReal then
        case Op of
          lxLess:
            if Result.Real < tmp.Real then
              Result.Real := 1
            else
              Result.Real := 0;
          lxLessOrEqual:
  ...
  begin
    ...
    begin
      ...
      if (Result.Real <> 0) and (tmp.Real <> 0) then
        Result.Real := 1
      else
        Result.Real := 0;
      Result._Type := tyReal;
  ...
  begin
    ...
    begin
      ...
      case Op of
        lxOr:
          if (Result.Real <> 0) or (tmp.Real <> 0) then
            Result.Real := 1
          else
            Result.Real := 0;
        lxXor:
          if (Result.Real <> 0) xor (tmp.Real <> 0) then
            Result.Real := 1
          else
            Result.Real := 0;
      end;
...
var
  ...
  begin
    Inc(Pos);
    if Lexemes^[Pos]._Type <> lxName then
      CheckError(true, 'имя массива', Lex2Str[Lexemes^[Pos]._Type])
    else
    begin
      Inc(Pos);
      if Lexemes^[Pos]._Type <> lxLSBracket then
      ...
      begin
        ...
        begin
          if Lexemes^[Pos]._Type <> lxComma then
            CheckError(true, 'запятая', Lex2Str[Lexemes^[Pos]._Type]);
          Inc(Pos);
          CheckExpr(Lexemes);
        end;
        Inc(Pos);
  ...
  begin
    if Lexemes^[Pos]._Type in [lxUndef, lxSemicolon] then
    begin
      Inc(Pos);
      Exit;
    end;

    if Lexemes^[Pos]._Type = lxArr then
      Arr
    else if Lexemes^[Pos]._Type <> lxName then
    begin
      CheckError(true, 'имя', Lex2Str[Lexemes^[Pos]._Type]);
      Exit;
    end
    ...
    begin
      Inc(Pos);
      if Lexemes^[Pos]._Type in [lxLBracket, lxLSBracket] then
      begin
        if Lexemes^[Pos]._Type = lxLBracket then
          Bracket := lxRBracket
        else
          Bracket := lxRSBracket;
        while Lexemes^[Pos]._Type <> Bracket do
        begin
          Inc(Pos);
          CheckExpr(Lexemes);
          if not(Lexemes^[Pos]._Type in [lxComma, Bracket]) then
            CheckError(true, 'запятая или скобка', Lex2Str[Lexemes^[Pos]._Type]);
        end;
        Inc(Pos);
      end;
      if Bracket = lxRBracket then
        Exit;
      if Lexemes^[Pos]._Type <> lxAssignment then
      begin
  ...
  begin
    Oper;
    while Lexemes^[Pos]._Type = lxSemicolon do
    begin
      Inc(Pos);
      Oper;
    end;
    if Lexemes^[Pos]._Type <> lxUndef then
      CheckError(false, 'Не хватает ; после оператора');
  end;
...
procedure ExecOperator(Lexemes: PLexemes; Vars: TVars); // Обработка списка операторов
  ...
  begin
    ...
    begin
      myVar := Vars[VarNo];
      myVar.Sizes.Clear;
      for i := 0 to myVar.Arr.Count - 1 do
        Dispose(PValue(myVar.Arr[i]));
      myVar.Arr.Clear;
    end
    else
    begin
    ...
    if (tmpExpr._Type <> tyReal) or (tmpExpr.Real < 0) then
    begin
...
const
  Funcs: array [1 .. 41] of Func = (
    ...
    (Name: 'Abs'; _Type: ftMath),
    (Name: 'Sign'; _Type: ftMath),

    // ADDED IN VERSION 3.2a
    (Name: 'Round'; _Type: ftMath),
    (Name: 'Int'; _Type: ftMath),
    (Name: 'Floor'; _Type: ftMath),
    (Name: 'Ceil'; _Type: ftMath),
...
var
  ...
  var
    ...
    procedure Skip;
    begin
      while CharInSet(Path[Cur], Space) and (Cur < Length(Path)) do
        Inc(Cur);
    end;
    ...
    begin
      Result := '';
      Skip;
      while (Cur <= Length(Path)) and (Path[Cur] <> '\') do
      begin
        Result := Result + Path[Cur];
        Inc(Cur);
      end;
      Inc(Cur);
      while CharInSet(Result[Length(Result)], Space) do
        Delete(Result, Length(Result), 1);
    end;

  begin
    if System.Pos(':', Path) > 0 then
      Error;
    Cur := 1;
    Skip;
    if Path[Cur] = '\' then
...
begin
  for i := Low(Funcs) to High(Funcs) do
    if UpperCase(Name) = UpperCase(Funcs[i].Name) then
      Break;

  if i > High(Funcs) then
  begin
  ...
  n := LowerCase(Name);
  if n = 'sin' then
    Result := MakeVal(Sin(P[1].Real * pi / 180));
  if n = 'cos' then
    Result := MakeVal(Cos(P[1].Real * pi / 180));
  if n = 'tan' then
    Result := MakeVal(Tan(P[1].Real * pi / 180));
  if n = 'arcsin' then
    Result := MakeVal(arcSin(P[1].Real) * 180 / pi);
  if n = 'arccos' then
    Result := MakeVal(arcCos(P[1].Real) * 180 / pi);
  if n = 'arctan' then
    Result := MakeVal(arcTan(P[1].Real) * 180 / pi);
  if n = 'sqr' then
    Result := MakeVal(Sqr(P[1].Real));
  if n = 'sqrt' then
    Result := MakeVal(Sqrt(P[1].Real));
  if n = 'abs' then
    Result := MakeVal(Abs(P[1].Real));
  if n = 'sign' then
...
//  end;
  ...
  begin
    ...
    try
      ...
      on EConvertError do
        RunTimeError('Строка ''' + P[1].Str + ''' не переводится в число');
    end;
  end;
  ...
  if n = 'calc' then
  begin
    if not(Params.Count in [1, 2]) or (P[1]._Type <> tyStr) then
      RunTimeError('Неправильный список параметров функции Calc');
    Lines := TStringList.Create;
    New(Lexs);
    XVars := TVars.Create;
    tmpPos := Pos;
  ...
  begin
    ...
    try
      if Trunc(P[2].Real) in [0, 2] then
        File_Rec.Strings.LoadFromFile(P[1].Str);
    except
  ...
  begin
    try
      ...
      begin
        File_Rec := Files[i];
        if File_Rec.ID = P[1].Real then
        begin
          if File_Rec.Write then
            File_Rec.Strings.SaveToFile(File_Rec.FileName);
  ...
  begin
    ...
    for i := 0 to Files.Count - 1 do
      ...
      begin
        File_Rec := Files[i];
        s := '';
        for j := 2 to Params.Count do
          if P[j]._Type = tyStr then
            s := s + P[j].Str
          else
            s := s + FloatToStr(P[j].Real);
      ...
      begin
        ...
        for j := 2 to Params.Count do
          if P[j]._Type = tyStr then
            ...
            s := s + FloatToStr(P[j].Real);
        // File_Rec.Strings.Insert(File_Rec.Pos, s);
        if File_Rec.Pos < File_Rec.Strings.Count then
          File_Rec.Strings[File_Rec.Pos] := s
        else
          File_Rec.Strings.Add(s);
        Inc(File_Rec.Pos);
      end;
    Result := MakeVal(0);
  end;
  ...
  if n = 'round' then
    Result := MakeVal(Round(P[1].Real));
  if n = 'int' then
    Result := MakeVal(Int(P[1].Real));
  if n = 'floor' then
    Result := MakeVal(Floor(P[1].Real));
  if n = 'ceil' then
    Result := MakeVal(Ceil(P[1].Real));
