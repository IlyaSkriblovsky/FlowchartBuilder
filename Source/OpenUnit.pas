...
var
  ...
  begin
    ...
    str:=DoRead;
    if str='ELLIPSE' then B.Block:=stBeginEnd;
    if str='RECT' then B.Block:=stStatement;
    if str='ROMB' then B.Block:=stIf;
    if str='PARAL' then B.Block:=stInOut;
    if str='CALL' then B.Block:=stCall;
    if str='COMMENT' then B.Block:=stComment;
    if str='CONFLUENCE' then B.Block:=stConfl;
    if str='INIT' then begin
                         B.Block:=stInit;
                         InitBlock:=B;
                         AlreadyInit:=true;
                         B.InitCode.Assign(Init);
                       end;
    if str='GLOB' then begin
                         B.Block:=stGlob;
                         GlobBlock:=B;
                         AlreadyGlob:=true;
  ...
  begin
    ...
    str:=DoRead;
    if str='VERT' then A._Type:=vert;
    if str='HORIZ' then A._Type:=horiz;
...
//    A.p:=StrToInt(str);
    ...
    do begin
         ...
         if str<>'NIL'
         then begin
                if str='NORTH' then A.Blocks[t].Port:=North;
                if str='EAST' then A.Blocks[t].Port:=East;
                if str='WEST' then A.Blocks[t].Port:=West;
                if str='SOUTH' then A.Blocks[t].Port:=South;
                str:=DoRead;
...
begin
  try
    ...
    while Copy(str, 1, 5)='BLOCK'
    do LoadBlock;
    Init.Free;
    Glob.Free;
    while Copy(str, 1, 5)='ARROW'
    do LoadArrow;
