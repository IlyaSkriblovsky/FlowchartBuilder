...
var
  ...
  begin
    ...
    WriteLn(F, B.Height);
    case B.Block of
      stBeginEnd: WriteLn(F, 'ELLIPSE');
      stStatement: WriteLn(F, 'RECT');
      stIf: WriteLn(F, 'ROMB');
      stInOut: WriteLn(F, 'PARAL');
      stCall: WriteLn(F, 'CALL');
      stGlob: WriteLn(F, 'GLOB');
      stInit: WriteLn(F, 'INIT');
      stComment: WriteLn(F, 'COMMENT');
      stConfl: WriteLn(F, 'CONFLUENCE');
    end;
    WriteLn(F, '  STATEMENT');
  ...
  begin
    ...
    WriteLn(F, A.p);
    for t:=atStart to atEnd
    do if A.Blocks[t].Block<>nil
       then begin
              case A.Blocks[t].Port of
                North: WriteLn(F, 'NORTH');
                 East: WriteLn(F, 'EAST');
                 West: WriteLn(F, 'WEST');
                South: WriteLn(F, 'SOUTH');
              end;
...
begin
  ...
  try
    ...
    then begin
           WriteLn(F, 'INIT');
           for i:=0 to InitBlock.InitCode.Count-1
           do WriteLn(F, InitBlock.InitCode[i]);
           WriteLn(F, '/INIT');
         end
    else WriteLn(F, 'NIL');
    if AlreadyGlob
    then begin
           WriteLn(F, 'GLOB');
           for i:=0 to GlobBlock.GlobStrings.Count-1
           do WriteLn(F, GlobBlock.GlobStrings[i]);
           WriteLn(F, '/GLOB');
         end
    else WriteLn(F, 'NIL');

    for i:=0 to ChildForm.BlockList.Count-1
    do TBlock(ChildForm.BlockList[i]).Tag:=i;
    if ChildForm.StartBlok<>nil
    then WriteLn(F, ChildForm.StartBlok.Tag)
    else WriteLn(F, -1);
    for i:=0 to ChildForm.BlockList.Count-1
    do SaveBlock(TBlock(ChildForm.BlockList[i]));
